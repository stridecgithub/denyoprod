...
@Component({
  selector: 'page-maps',
  templateUrl: 'mapsaction.html',
  providers: [Config]
})
export class MapsActionPage {
  mapaction: GoogleMap;
  ...
  constructor(private conf: Config, private network: Network, public platform: Platform, private googleMaps: GoogleMaps, public http: Http, public navCtrl: NavController,
  ...
  doUser() {
    ...
    let res;
    console.log(url);
    console.log("doUser mapactions.ts:" + url);
  ...
  loadMap(val) {
    ...
    this.mapElement = document.getElementById('mapaction');
    ...
    if (val == 0) {
      ...
      this.mapaction = this.googleMaps.create(this.mapElement, mapOptions);
    }
    ...
    this.mapaction.one(GoogleMapsEvent.MAP_READY)
      .then(() => {
